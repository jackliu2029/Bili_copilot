...
- 本项目仅供个人学习与技术研究，不提供公共服务或商业化支持。
- 使用者应自行遵守 B站用户协议、相关版权规则以及所在司法管辖区的法律法规。
- 本项目仅处理用户明确传入的单个视频链接，不鼓励批量抓取、自动采集或大规模爬取。
- 提取出的字幕原文属于原视频版权方，请勿在公开渠道分发完整字幕全文。
- 本项目不会主动绕过平台限制；如需登录态字幕，必须由用户通过本机 Cookie 文件显式授权。
